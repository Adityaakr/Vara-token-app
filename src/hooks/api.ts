...
export function useTokenQueries(): TokenMetaQueries {
  ...
  // Wrap refetchTotalSupply to handle the promise properly
  const handleRefetchTotalSupply = async () => {
    try {
      await refetchTotalSupply();
    } catch (error) {
      console.error('Error refetching total supply:', error);
    }
  };
  ...
  return {
    ...
    refetchTotalSupply: handleRefetchTotalSupply,
  };
}
 ...
 */
export function useBalanceOfQuery(address: `0x${string}`, options?: { enabled: boolean }) {
  const { data: program } = useProgramInstance();
